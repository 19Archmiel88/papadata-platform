...
**Odpowiedzialność szczegółowa:** `RecommendationCard` odpowiada za prezentowanie rekomendacji, decyzji, śladów dowodowych, zgód i operacji wymagających audytu. W dokumentacji 1.0 komponent jest traktowany jako gotowy element kontraktu projektowego: ekran może go skonfigurować, ale nie może przejąć jego semantyki ani zduplikować jego zachowania w lokalnym kodzie.

**Dane wejściowe:** minimalny model danych komponentu obejmuje: `componentId, state, density, testId, recommendationId, evidence, risk, decisionState, auditTrail`. Dane biznesowe muszą zostać przygotowane w view modelu ekranu, a komponent otrzymuje już przetworzone wartości, status jakości danych i teksty do pokazania użytkownikowi.
...
**Konsumenci:** komponent jest powiązany z następującymi ekranami lub uzasadnieniem katalogowym: `30.02 | 30.11 | 30.14 | 31.05 | 31.07 | 34.02 | 34.07 | 35.02`. Jeżeli nowy ekran chce użyć komponentu inaczej, najpierw aktualizuje ten dokument, macierz `ekran-komponent.csv` i Storybook.
...
`RecommendationCard` jest komponentem bazowym używanym przez wiele ekranów. Definiuje jeden wspólny model semantyczny, jeden kontrakt propsów i jeden zestaw stanów. Ekran może zmienić treść, ale nie może dopisać lokalnego API komponentu.
...
Każde zdarzenie używa `RecommendationCardEvent`, zawiera `componentId`, nazwę akcji, opcjonalny `screenId` i `correlationId`. Payload telemetryczny nie przenosi PII, sekretów ani pełnych wartości tokenów integracyjnych.
...
|---|---|---|
| `07-centrum-dowodzenia/30-02-kolejka-uwagi.md` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `30.02` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `30.11` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `30.14` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `31.05` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `31.07` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `34.02` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
| `34.07` | konsument docelowy | komponent wymagany przez kompozycję ekranu albo przez test stanu |
...
Ten plik jest aliasem dokumentacyjnym i nie tworzy drugiego komponentu `RecommendationCard`. Jedynym kanonicznym kontraktem jest `04-komponenty-domenowe/recommendation-card.md`, a kompilowany plik propsów to `contracts/components/recommendationcard.ts`. Ekrany, macierze i testy muszą wskazywać wyłącznie kanoniczny komponent, aby uniknąć równoległych wersji odpowiedzialności.
