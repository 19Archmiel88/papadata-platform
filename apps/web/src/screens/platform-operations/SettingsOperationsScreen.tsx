...
const labels:Record<SettingsView,[string,string]>={profile:['Konto','Account'],security:['Bezpieczeństwo','Security'],organization:['Firma','Organization'],workspace:['Workspace','Workspace'],team:['Zespół','Team'],analytics:['Analityka','Analytics'],ai:['Papa AI','Papa AI'],notifications:['Powiadomienia','Notifications'],privacy:['Prywatność','Privacy'],audit:['Audyt','Audit']};
...
export function SettingsOperationsScreen({data,state,problem,onReload,onSave,children,demo}:SettingsOperationsScreenProps){
 ...
 return <OperationsFrame title={t('Ustawienia','Settings')} description={t('Ustawienia konta, organizacji i workspace mają osobne zakresy zapisu.','Account, organization and workspace settings have separate write scopes.')} onReload={onReload?()=>{if(dirty)setNextView('reload');else onReload();}:undefined} demo={demo}>
 <div className="pd-settings-layout"><aside className="pd-settings-navigation">{[
 {title:t('Osobiste','Personal'),ids:['profile','security','notifications']},
 {title:t('Organizacja','Organization'),ids:['organization','workspace','team']},
 {title:t('Dane i kontrola','Data and control'),ids:['analytics','ai','privacy','audit']}
 ].map(group=><section key={group.title}><h2>{group.title}</h2><ProductViewNav label={group.title} active={view} items={group.ids.map(id=>({id:id as SettingsView,label:t(...labels[id as SettingsView])}))} onChange={change}/></section>)}</aside>
 <div className="pd-settings-content"><ProductDataState state={state} problem={problem} onRetry={onReload}>{data&&<>{doc?<ProductSectionFrame icon={view==='profile'?'customers':'data'} title={t(...labels[view])} description={doc.scope==='self'?t('Preferencje Twojego konta.','Your account preferences.'):doc.scope==='tenant'?t('Dane wspólne dla organizacji.','Shared organization details.'):t('Konfiguracja bieżącego obszaru roboczego.','Current workspace configuration.')}><SettingEditor key={`${doc.section}:${doc.version}`} document={doc} onDirty={setDirty} onSave={onSave} demo={demo}/></ProductSectionFrame>:['profile','organization','workspace','analytics','notifications'].includes(view)?<p>{t('Brak danych tej sekcji. Odśwież dane zamiast zapisywać puste wartości.','No data for this section. Refresh rather than saving empty values.')}</p>:children}</>}</ProductDataState></div></div>
 <OperationsDialog open={nextView!==null} title={t('Niezapisane zmiany','Unsaved changes')} onClose={()=>setNextView(null)}><p>{t('Zmiany formularza nie zostały zapisane. Odrzucić je i przejść dalej?','The form has unsaved changes. Discard them and continue?')}</p><div className="pd-operations__actions"><Button variant="secondary" onClick={()=>setNextView(null)}>{t('Zostań','Stay')}</Button><Button onClick={()=>{if(nextView){setDirty(false);if(nextView==='reload')onReload?.();else update({settingsView:nextView});setNextView(null);}}}>{t('Odrzuć zmiany','Discard changes')}</Button></div></OperationsDialog>
 </OperationsFrame>;
}
function SettingEditor({document:doc,onSave,onDirty,demo}:{document:SettingDocument;onDirty:(value:boolean)=>void;onSave:SettingsOperationsScreenProps['onSave'];demo?:boolean}){
 ...
 const names:Record<string,[string,string]>={name:['Nazwa','Name'],language:['Język','Language'],theme:['Motyw','Theme'],timezone:['Strefa czasowa','Time zone'],revenueGoal:['Cel przychodu','Revenue goal'],currency:['Waluta celu','Goal currency'],digestOptIn:['Zapisz preferencję podsumowań','Store digest preference'],integrationAlerts:['Preferencja alertów integracji','Integration alert preference'],billingAlerts:['Preferencja alertów rozliczeń','Billing alert preference']};
 async function save(){if(!onSave||busy)return;setBusy(true);setError(null);const body=JSON.stringify(values);if(request.current?.body!==body)request.current={body,id:safeRandomUUID()};try{await onSave(doc.section,{requestId:request.current.id,expectedVersion:doc.version,values});setSuccess(true);setConfirm(false);onDirty(false);}catch(cause){setError(cause instanceof Error?cause.message:t('Zapis nie powiódł się','Save failed'));}finally{setBusy(false);}}
 ...
 {doc.version===0&&<p>{t('Wartości początkowe. Ten zestaw preferencji nie został jeszcze zapisany.','Initial values. These preferences have not yet been saved.')}</p>}
 ...
 {doc.section==='analytics'&&<p>{t('Cel i waluta są konfiguracją planu workspace. Zapis nie przelicza historycznych danych ani nie zmienia ich strefy czasowej.','Goal and currency configure workspace planning. Saving does not recalculate historical data or change its time zone.')}</p>}
 {doc.section==='notifications'&&<p>{t('Zapis dotyczy preferencji. Ta paczka nie uruchamia nowych wysyłek e-mail i nie wyłącza obowiązkowych komunikatów bezpieczeństwa.','Saving stores preferences. This package does not activate new email deliveries or suppress mandatory security messages.')}</p>}
 {doc.section==='organization'&&<p>{t('Nazwa wyświetlana organizacji. Dane prawne i podatkowe na fakturach zmienisz u dostawcy rozliczeń.','Organization display name. Legal and tax invoice details are maintained with the billing provider.')}</p>}
 ...
 <OperationsDialog open={confirm} title={t('Potwierdź zmianę ustawień','Confirm settings change')} busy={busy} onClose={()=>setConfirm(false)}><dl>{Object.entries(values).filter(([key,value])=>value!==doc.values[key]).map(([key,value])=><div key={key}><dt>{t(...(names[key]??[key,key]))}</dt><dd>{String(doc.values[key])} &rarr; {String(value)}</dd></div>)}</dl><OperationError message={error}/><Button type="button" disabled={busy} onClick={()=>void save()}>{busy?t('Zapisywanie','Saving'):t('Potwierdź zapis','Confirm save')}</Button></OperationsDialog>
 </form>;
}
