...
export class AccessLifecycleService{
 ...
 async read(p:RequestPrincipal):Promise<AccessLifecycleStatus>{return this.scoped(p,async c=>{
  ...
  const company=(await c.query<{version:number;profile:CompanyProfile}>('SELECT version,profile FROM app.access_company_profiles WHERE tenant_id=$1 AND workspace_id=$2',[p.tenantId,p.workspaceId])).rows[0];
 ...
 async company(p:RequestPrincipal,body:unknown):Promise<object>{
  ...
  const b=object(body);onlyKeys(b,['requestId','expectedVersion','values']);const id=uuid(b.requestId),v=version(b.expectedVersion),raw=object(b.values);onlyKeys(raw,['legalName','vatId','street','city','postalCode','country']);
  ...
  return this.scoped(p,c=>operation(c,p,id,'access.company.save',{v,values},async()=>{
   ...
   await c.query(`INSERT INTO app.access_company_profiles(tenant_id,workspace_id,version,profile,updated_by) VALUES($1,$2,$3,$4::jsonb,$5) ON CONFLICT(tenant_id,workspace_id) DO UPDATE SET version=EXCLUDED.version,profile=EXCLUDED.profile,updated_by=EXCLUDED.updated_by,updated_at=now()`,[p.tenantId,p.workspaceId,v+1,JSON.stringify(values),p.userId]);return {version:v+1,values,source:'manual'};
  }));
 }
