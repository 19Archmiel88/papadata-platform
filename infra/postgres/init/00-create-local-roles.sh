...
ALTER ROLE papadata_app WITH LOGIN PASSWORD :'app_password' NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION;
ALTER ROLE papadata_test WITH LOGIN PASSWORD :'test_password' NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION;
...
SELECT format('GRANT CONNECT ON DATABASE %I TO papadata_migrator, papadata_app', :'papadata_database')\gexec
