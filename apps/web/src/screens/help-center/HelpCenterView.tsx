...
export function HelpCenterView({tickets=[],total=tickets.length,state='ready',problem,onReload,onSubmit,demo=false,pageSize=50,deliveryWarning,ticketDetail,ticketState='ready',ticketProblem,onReloadTicket,onTicketCommand}:HelpCenterViewProps){
 ...
 return <div className="pd-product-data" data-testid="help-center-bi-page">
  ...
  <div className="pd-product-data__toolbar"><label>{t('Szukaj w bazie wiedzy','Search the knowledge base')}<input type="search" value={search} onChange={e=>update({helpSearch:e.target.value})}/></label><label>{t('Temat','Topic')}<select value={category} onChange={e=>update({helpCategory:e.target.value})}>{[['all',t('Wszystkie','All')],['data',t('Dane i integracje','Data and integrations')],['analysis',t('Analizy','Analytics')],['access',t('Dostęp','Access')],['assistant',t('Papa Asystent','Papa Assistant')]].map(([id,label])=><option value={id} key={id}>{label}</option>)}</select></label></div>
  <ProductSectionFrame icon="help" title={t('Procedury i wyjaśnienia','Guidance and explanations')} description={t('Instrukcje produktu, bez fikcyjnych wyników audytu lub SLA.','Product guidance, without fabricated audit results or SLAs.')}>
  ...
  <ProductSectionFrame icon="assistant" title={t('Pomoc z kontekstem analizy','Help in the context of your analysis')} description={t('Papa Asystent korzysta z rozmowy i koszyka kontekstu. Nie otwieramy oddzielnego pozornego Copilota.','Papa Assistant uses your conversation and context basket. There is no separate simulated Copilot.')}><Button variant="secondary" onClick={()=>navigate(contextualProductLink(productRoutes.assistant,{helpTopic:requested,prompt:search||null}))}>{t('Otwórz Papa Asystenta','Open Papa Assistant')}</Button></ProductSectionFrame>
  <ProductSectionFrame icon="data" title={t('Sprawy tego workspace','Workspace requests')} description={t(`Ostatnie ${tickets.length} z ${total} spraw. Prośba o konsultację nie jest rezerwacją terminu.`,`Latest ${tickets.length} of ${total} requests. A consultation request is not a booked appointment.`)} actions={onReload?<Button variant="ghost" size="small" onClick={onReload}>{t('Odśwież','Refresh')}</Button>:null}>
   ...
   <ProductDataState state={state} problem={problem} onRetry={onReload}><ExplorerTable rows={tickets} manualSearch searchQuery={ticketSearch} onSearchQueryChange={value=>update({ticketSearch:value,ticketPage:null})} canExport={false} ariaLabel={t('Sprawy wsparcia','Support requests')} searchText={row=>`${row.number} ${row.subject}`} columns={[
