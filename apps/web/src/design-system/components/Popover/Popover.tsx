...
import { joinClassNames, resolveDescribedBy } from '../Field/fieldUtils';
import type { OverlayCloseReason } from '../OverlayRoot';
import {
  focusFirstElement,
  mergeRefs,
  useAnchoredPosition,
...
export type PopoverProps = Omit<HTMLAttributes<HTMLDivElement>, 'children'> & {
  ...
  readonly onOpenChange?: ((open: boolean, reason: OverlayCloseReason) => void) | undefined;
  readonly open: boolean;
  readonly placement: 'top' | 'right' | 'bottom' | 'bottom-start' | 'bottom-end' | 'left';
...
export const Popover = forwardRef<HTMLDivElement, PopoverProps>(function Popover(
  ...
  useEffect(() => {
    ...
    const isInsidePopover = (target: Node) =>
  ...
  const triggerElement = isValidElement(trigger)
    ? cloneElement(trigger as ReactElement<any>, {
        ...
        onClick: (event: React.MouseEvent<HTMLElement>) => {
          (triggerProps.onClick as ((event: React.MouseEvent<HTMLElement>) => void) | undefined)?.(
            event,
          );
        ...
        onKeyDown: (event: ReactKeyboardEvent<HTMLElement>) => {
          (
            triggerProps.onKeyDown as ((event: ReactKeyboardEvent<HTMLElement>) => void) | undefined
          )?.(event);
  ...
  return (
    <div
      ...
      className={joinClassNames('pd-popover', className)}
    >
      <span className="pd-popover__trigger">{triggerElement}</span>
      ...
      {open && portalRoot
        ? createPortal(
            <div
              ...
              className={joinClassNames('pd-overlay-surface', 'pd-popover__panel')}
            ...
            >
              {title || description ? (
                <header className="pd-overlay-surface__header">
                  <div className="pd-overlay-surface__heading">
                    {title ? (
                      <h3 className="pd-overlay-surface__title" id={titleId}>
                        {title}
                      </h3>
                    ) : null}
                    {description ? (
                      <p className="pd-overlay-surface__description" id={descriptionId}>
                        {description}
                      </p>
                    ) : null}
                  </div>
              ...
              {children ? (
                <div className="pd-overlay-surface__body" id={bodyId}>
                  {children}
                </div>
              ) : null}
              ...
              {actionLabel ? (
                <footer className="pd-overlay-surface__footer">
                  <Button
                    data-autofocus="true"
                    onClick={() => {
                      onOpenChange?.(false, 'primary-action');
                    }}
                    size="small"
                    type="button"
                    variant="primary"
