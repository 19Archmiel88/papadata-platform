...
component_id: ReconciliationPanel
---
# ReconciliationPanel

## Cel i odpowiedzialność
`ReconciliationPanel` rozwiązuje jeden określony problem interfejsu i nie przejmuje odpowiedzialności ekranu ani domenowego API. Kontrakt jest stanem docelowym wymagającym implementacji i testów.
...
`contracts/components/reconciliationpanel.ts` re-eksportuje `ReconciliationPanelProps` z `contracts/domain-component-contracts.ts`; nie istnieje drugi konkurencyjny interfejs.

| Pole / kontrakt | Typ | Reguła |
|---|---|---|
| `ReconciliationPanelProps` | import z `domain-component-contracts.ts` | Szczegółowe pola domenowe są zdefiniowane w jednym pliku kanonicznym. |
