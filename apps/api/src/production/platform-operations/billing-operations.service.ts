...
import { readStripeBillingConfig, StripeBillingClient, stripeRecord, stripeTimestamp, stripeSafeUrl, stripeId } from '@papadata/integrations';
...
export class BillingOperationsService {
 ...
 async read(p:RequestPrincipal,cursor?:string):Promise<BillingOverview>{
  ...
  if(subscription.stripeCustomerId){
   try{
    ...
    result.invoices=collection.data.map((value:unknown):BillingInvoiceView=>{
     ...
     return {id,number:typeof invoice.number==='string'?invoice.number:null,currency:String(invoice.currency).toUpperCase(),total:invoice.total,due:invoice.amount_remaining,status:String(invoice.status??'unknown'),createdAt:stripeTimestamp(invoice.created)??'',dueAt:stripeTimestamp(invoice.due_date),pdfUrl:stripeSafeUrl(invoice.invoice_pdf,['pay.stripe.com','invoice.stripe.com']),paymentUrl:stripeSafeUrl(invoice.hosted_invoice_url,['invoice.stripe.com']),ksefStatus:'not_connected'};
 ...
 async session(p:RequestPrincipal,value:unknown):Promise<BillingSessionResult>{
  ...
  const intent=await this.db.withTenantWorkspace(p.tenantId,p.workspaceId,async c=>{
   ...
   const configurationHash=createHash('sha256').update(JSON.stringify({mode:client.config.mode,price:offer.priceId,customer:live?.stripe_customer_id??null,returnUrl})).digest('hex');
   ...
   const params:Record<string,string>={mode:'subscription','line_items[0][price]':offer.priceId,'line_items[0][quantity]':'1',client_reference_id:`${p.tenantId}:${p.workspaceId}`,'metadata[planId]':offer.plan,'subscription_data[metadata][planId]':offer.plan,success_url:returnUrl,cancel_url:returnUrl,expires_at:String(expires),...(live?.stripe_customer_id?{customer:live.stripe_customer_id}:{})};
