...
export function SettingsGovernancePanel({data}:{data:GovernanceView}){
 ...
 return <ProductSectionFrame icon="assistant" title={t('Papa AI: zasady wykonania','Papa AI: execution policy')} description={t('Rzeczywista odpowiedz backendu. Brak przelacznika nie oznacza automatycznej zgody na narzedzia.','Actual backend response. An absent switch does not imply automatic tool approval.')}>
 <dl className="pd-operations__facts"><div><dt>{t('Tryb','Mode')}</dt><dd>{data.mode}</dd></div><div><dt>{t('Akceptacja skutkow zewnetrznych','Approval of external effects')}</dt><dd>{data.approval?t('wymagana','required'):t('niewymagana wedlug serwera','not required by server')}</dd></div><div><dt>{t('Wykonanie','Execution')}</dt><dd>{data.execute}</dd></div><div><dt>{t('Kompensacja','Compensation')}</dt><dd>{data.rollback}</dd></div><div><dt>{t('Izolacja tenant/workspace','Tenant/workspace scope')}</dt><dd>{data.scopeRequired?t('wymagana','required'):t('niepotwierdzona','not confirmed')}</dd></div></dl>
 <p>{t('Polityka pamieci i kontekstu jest edytowana ponizej po odczycie uprawnien. To nie jest zgoda na wykonanie zewnetrznych AI Actions.','Memory and context policy is edited below after permissions are loaded. This does not authorize external AI Actions.')}</p>
 </ProductSectionFrame>;
}
