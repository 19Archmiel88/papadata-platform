...
def validate(root:Path):
 ...
 # canonical documents registry: exact document set + title + SHA-256 integrity
 document_registry_path=root/'rejestry/documents.csv'
 document_rows=read_csv(document_registry_path) if document_registry_path.exists() else []
 document_map={r.get('path',''):r for r in document_rows if r.get('path')}
 actual_documents={str(p.relative_to(spec)).replace('\\','/'):p for p in docs}
 if len(document_map)!=len(document_rows):err('DOCUMENT_REGISTRY_DUPLICATE','duplicate or empty document paths')
 missing_documents=sorted(set(actual_documents)-set(document_map))
 extra_documents=sorted(set(document_map)-set(actual_documents))
 if missing_documents:err('DOCUMENT_REGISTRY_MISSING',f'{len(missing_documents)} missing; examples {missing_documents[:8]}')
 if extra_documents:err('DOCUMENT_REGISTRY_EXTRA',f'{len(extra_documents)} extra; examples {extra_documents[:8]}')
 for rel,p in actual_documents.items():
  row=document_map.get(rel)
  if not row:continue
  text=p.read_text(encoding='utf-8',errors='ignore')
  h1=next((line[2:].strip() for line in text.splitlines() if line.startswith('# ')),p.stem)
  if row.get('title','')!=h1:err('DOCUMENT_REGISTRY_TITLE',rel)
  if row.get('sha256','')!=sha(p):err('DOCUMENT_REGISTRY_HASH',rel)
  if not row.get('words','').isdigit():err('DOCUMENT_REGISTRY_WORDS',rel)
 checks['document_registry']={'rows':len(document_rows),'missing':len(missing_documents),'extra':len(extra_documents)}
