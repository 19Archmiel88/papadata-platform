...
export function renderExample(contract) {
  const groups = [
    ["NODE_ENV","POSTGRES_PASSWORD","PAPADATA_APP_PASSWORD","PAPADATA_PLATFORM_PASSWORD","PAPADATA_TEST_PASSWORD","REDIS_PASSWORD","MINIO_ROOT_USER","MINIO_ROOT_PASSWORD"],
    ["DATABASE_URL","SCHEDULER_DATABASE_URL","REDIS_URL","REDIS_CA_BASE64"],
    ["PAPADATA_STORAGE_DRIVER","PAPADATA_STORAGE_BUCKET","PAPADATA_STORAGE_ENDPOINT","PAPADATA_STORAGE_ACCESS_KEY","PAPADATA_STORAGE_SECRET_KEY"],
    ["PAPADATA_API_AUTH_ACTIVE_SECRET","PAPADATA_API_AUTH_PREVIOUS_SECRET","PAPADATA_API_AUTH_ISSUER","PAPADATA_API_AUTH_AUDIENCE","PAPADATA_API_AUTH_SESSION_STORE","PAPADATA_API_AUTH_SESSION_REDIS_PREFIX","PAPADATA_INFRASTRUCTURE_AUTH_TOKEN","MFA_ENCRYPTION_KEY"],
    ["BFF_PORT","API_ORIGIN","BFF_ALLOWED_ORIGINS","BFF_PUBLIC_HOSTS","BFF_COOKIE_SECRET","BFF_COOKIE_PREVIOUS_SECRET","BFF_CSRF_SECRET","BFF_INTERNAL_AUTH_ACTIVE_SECRET","BFF_INTERNAL_AUTH_PREVIOUS_SECRET","BFF_INTERNAL_AUTH_ISSUER","BFF_INTERNAL_AUTH_AUDIENCE","BFF_SESSION_STORE","BFF_SESSION_REDIS_PREFIX","BFF_RATE_LIMIT_MAX","BFF_RATE_LIMIT_WINDOW_MS"],
...
export async function resolveEnvironment(contract, localContract, existing = new Map(), options = {}) {
  const values = new Map();
  ...
  async function resolveName(name, stack = []) {
    ...
    if (stack.includes(name)) throw new Error(`Circular env contract reference: ${[...stack, name].join(" -> ")}`);
    ...
    if (source.kind === "literal") {
      ...
      if (typeof value !== "string") throw new Error(`Contract path ${source.path} does not resolve to text.`);
