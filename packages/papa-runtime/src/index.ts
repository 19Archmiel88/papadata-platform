  ...
  if (options.parentConversationId && conversationId && !branch) {
    await options.repository.upsertAssistantCase({
      ...
      caseType: readAllowedRecordString(
        options.snapshot,
        "caseType",
        ["action", "analysis", "anomaly", "data_quality", "decision", "opportunity", "report", "risk"],
        "analysis",
      ) as "action" | "analysis" | "anomaly" | "data_quality" | "decision" | "opportunity" | "report" | "risk",
      ...
      status: readAllowedRecordString(
        ...
        "detected",
      ) as "actioned" | "awaiting_approval" | "detected" | "dismissed" | "monitoring" | "needs_data" | "recommendation_ready" | "resolved" | "triaging",
      tenantId: options.tenantId,
      title: options.title,
      workspaceId: options.workspaceId,
    });
