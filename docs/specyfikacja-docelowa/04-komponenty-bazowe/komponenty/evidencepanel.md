...
**Odpowiedzialność szczegółowa:** `EvidencePanel` odpowiada za jedną powtarzalną odpowiedzialność UI, którą ekran konsumuje bez tworzenia lokalnego wariantu. W dokumentacji 1.0 komponent jest traktowany jako gotowy element kontraktu projektowego: ekran może go skonfigurować, ale nie może przejąć jego semantyki ani zduplikować jego zachowania w lokalnym kodzie.
...
`EvidencePanel` jest komponentem bazowym używanym przez wiele ekranów. Definiuje jeden wspólny model semantyczny, jeden kontrakt propsów i jeden zestaw stanów. Ekran może zmienić treść, ale nie może dopisać lokalnego API komponentu.
...
Każde zdarzenie używa `EvidencePanelEvent`, zawiera `componentId`, nazwę akcji, opcjonalny `screenId` i `correlationId`. Payload telemetryczny nie przenosi PII, sekretów ani pełnych wartości tokenów integracyjnych.
...
Ten plik jest aliasem dokumentacyjnym i nie tworzy drugiego komponentu `EvidencePanel`. Jedynym kanonicznym kontraktem jest `04-komponenty-domenowe/evidence-panel.md`, a kompilowany plik propsów to `contracts/components/evidencepanel.ts`. Ekrany, macierze i testy muszą wskazywać wyłącznie kanoniczny komponent, aby uniknąć równoległych wersji odpowiedzialności.
