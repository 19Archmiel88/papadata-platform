import type { ShellCommandResult, ShellNavigationGroup } from './shellTypes';
...
const runtimeNavigation: readonly RuntimeGroup[] = [
  {
    ...
    items: [
      {
        capabilities: ['analytics.read', 'analytics.command_center.read', 'analytics.metrics.read'],
        icon: 'home',
        id: 'command-center',
        label: 'Przegląd',
      ...
      {
        capabilities: ['decisions.read', 'decisions.manage'],
        icon: 'decisions',
        id: 'decisions',
        label: 'Decyzje',
  ...
  },
  {
    ...
    items: [
      {
        capabilities: ['analytics.read', 'campaigns.read'],
        icon: 'trend',
        id: 'campaigns',
      ...
      {
        capabilities: ['analytics.read', 'products.read'],
        icon: 'products',
        id: 'products',
        label: 'Produkty',
        path: '/app/products',
      ...
      {
        capabilities: ['analytics.read', 'customers.read'],
        icon: 'customers',
        id: 'customers',
        label: 'Klienci',
        path: '/app/customers',
      ...
      {
        capabilities: ['analytics.read', 'traffic.read'],
        icon: 'trend',
        id: 'traffic',
  ...
  },
  {
    ...
    items: [
      {
        capabilities: ['ai.assistant.run', 'ai.use', 'ai.read'],
        icon: 'assistant',
        id: 'papa',
        label: 'Zapisane raporty',
        path: '/app/papa',
      },
      {
        capabilities: [
  ...
  },
  {
    ...
    items: [
      {
        capabilities: ['settings.read', 'settings.manage', 'workspace.read'],
        icon: 'security',
        id: 'settings',
        label: 'Ustawienia',
      ...
      {
        capabilities: ['help.read', 'workspace.read'],
        icon: 'help',
        id: 'help',
        label: 'Pomoc i konsultacje',
...
function hasAnyCapability(capabilities: readonly string[], required: readonly string[]): boolean {
  if (required.length === 0) return true;
  const available = expandRuntimeCapabilities(capabilities);
  return required.some((capability) => available.has(capability));
}

function expandRuntimeCapabilities(capabilities: readonly string[]): ReadonlySet<string> {
