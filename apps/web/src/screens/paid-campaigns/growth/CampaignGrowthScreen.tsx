...
export function CampaignGrowthScreen({ data, state, problem, onReload, onSavePlan, onExport, exportBusy, exportProblem, demo = false, initialView = 'wynik' }: CampaignGrowthScreenProps) {
    ...
    <header className="pd-product-data__head"><div><h1>{t('Kampanie płatne', 'Paid campaigns')}</h1><p>{t('Wynik, wiarygodność pomiaru i kontrola planu.', 'Performance, measurement confidence and plan control.')}</p></div><div className="pd-product-data__toolbar"><ProductDateControl label={t('Okres kampanii', 'Campaign period')} allowFuture={view === 'budzet'} clearParams={['creativeId', 'campaignDetail']}/><Button variant="secondary" size="small" onClick={onReload} disabled={state === 'loading'}>{t('Odśwież dane', 'Refresh data')}</Button></div></header>
    ...
    <AnalysisScope summary={[
      params.get('channel') === 'google_ads' ? 'Google Ads' : params.get('channel') === 'meta_ads' ? 'Meta Ads' : t('Wszystkie kanały', 'All channels'),
      data?.choices.campaigns.find(row => row.id === params.get('campaignId'))?.name ?? (params.get('campaignId') ? t('Wybrana kampania', 'Selected campaign') : t('Wszystkie kampanie', 'All campaigns')),
      params.get('currency') || currency || t('Waluty osobno', 'Separate currencies'),
    ].join(' · ')} actions={<>
      <Button variant="secondary" size="small" onClick={() => navigate(contextualProductLink(productRoutes.reports, { reportTemplate: 'campaigns' }))}>{t('Przygotuj raport', 'Prepare report')}</Button>
      <Button variant="ghost" size="small" onClick={() => navigate(contextualProductLink(productRoutes.assistant))}>{t('Papa Asystent', 'Papa Assistant')}</Button>
    </>}>
    ...
    <ProductDataState provenance={data?{source:t('Połączone konta reklamowe / GA4 / sklep', 'Connected ad accounts / GA4 / store'),synchronizedAt:data.scope.synchronizedAt,calculatedAt:data.scope.calculatedAt,demo:demo,limitations:data.limitations.map(item => t(item.pl, item.en))}:undefined} state={displayState} problem={problem ?? (data?.scope.readiness === 'empty' ? t('Nie ma obserwacji w tym zakresie. Brak danych nie oznacza zerowych wydatków.', 'No observations in this range. Missing data does not imply zero spending.') : undefined)} onRetry={onReload}>
      {data && <>
      {(view === 'wynik' || view === 'kampanie') && <>
        ...
        {currency && <><div className="pd-product-data__chart" role="img" aria-label={t('Trend kampanii; tabela danych poniżej', 'Campaign trend; data table below')}><ResponsiveContainer width="100%" height="100%"><LineChart data={points}><CartesianGrid vertical={false} stroke="var(--pd-separator)"/><XAxis dataKey="date" minTickGap={48} tickFormatter={value=>String(value).slice(5).replace('-','/')} tickLine={false} axisLine={false}/><YAxis tickLine={false} axisLine={false} width={56}/><Tooltip contentStyle={{background: 'var(--pd-surface)', border: '1px solid var(--pd-separator-strong)', borderRadius: 10, color: 'var(--pd-text)'}} labelStyle={{color: 'var(--pd-text)'}} /><Line isAnimationActive={false} type="monotone" dataKey="spend" name={t('Wydatki', 'Spend')} stroke="var(--pd-growth-accent)" dot={false} connectNulls={false}/><Line isAnimationActive={false} type="monotone" dataKey="revenue" name={t('Wartość raportowana', 'Reported value')} stroke="var(--pd-growth-secondary)" dot={false} connectNulls={false}/></LineChart></ResponsiveContainer></div><details><summary>{t('Tabela trendu', 'Trend table')}</summary><div className="pd-product-data__table-wrap"><table><thead><tr><th>{t('Data', 'Date')}</th><th>{t('Wydatek', 'Spend')}</th><th>{t('Wartość', 'Value')}</th></tr></thead><tbody>{points.map(point => <tr key={point.date}><th scope="row">{point.date}</th><td>{growthMoney(point.spend, currency, language)}</td><td>{growthMoney(point.revenue, currency, language)}</td></tr>)}</tbody></table></div></details></>}</section>}
