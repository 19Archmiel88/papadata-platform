// Storybook-only concept showcases retained separately from the canonical product screen.
import type {
  FormEvent,
} from 'react';
import {
...
export function PapaLabWorkbench({
  ...
  const [activeLayer, setActiveLayer] = useState<PapaLabWorkbenchLayerId | null>(null);
...
function PapaLabLayer({
  ...
  readonly layer: PapaLabWorkbenchLayerId;
  ...
  const layerTitle = layerLabel(layer);

  return (
    <aside aria-label={layerTitle} className={`pd-pal-layer pd-pal-layer--${layer}`}>
      <header className="pd-pal-layer__header">
        <div>
          <span>Warstwa robocza</span>
          <h2>{layerTitle}</h2>
        </div>
        <button aria-label={`Zamknij ${layerTitle}`} onClick={onClose} type="button">
          Zamknij
        </button>
      </header>

      <div className="pd-pal-layer__body">
        {layer === 'sidebar' ? <PapaLabSidebar onNewAnalysis={onNewAnalysis} /> : null}
        {layer === 'context' ? (
        ...
        {layer === 'evidence' ? <PapaLabEvidenceInspector /> : null}
        {layer === 'quality' ? <PapaLabQualityInspector runState={runState} /> : null}
        {layer === 'actions' ? (
        ...
        {layer === 'library' ? <PapaLabEmbeddedReportCanvas /> : null}
      </div>

      {layer !== 'sidebar' && layer !== 'library' ? (
        <div className="pd-pal-layer__tabs" aria-label="Szybka zmiana inspektora" role="tablist">
...
function PapaLabCanvas({
  ...
  readonly activeLayer: PapaLabWorkbenchLayerId | null;
  ...
  readonly onLayerChange: (layer: PapaLabWorkbenchLayerId | null) => void;
  ...
  const activeModeDefinition = papaLabWorkbenchModes.find((mode) => mode.id === activeMode) ?? papaLabWorkbenchModes[0];
  const response = papaLabModeResponses[activeMode];

  return (
    <section aria-label="Laboratorium Papa Asystenta" className="pd-pal-decision-lab">
      <header className="pd-pal-lab-hero">
        <div>
          <span>Laboratorium Papa Asystenta</span>
          <h1>Dlaczego konwersja checkout mobile spadła?</h1>
          <p>Zakres: ostatnie 7 dni · Źródła: orders, sessions, payments, erp_stock · Pewność: wysoka · Tryb: {activeModeDefinition.label}</p>
        </div>
        <div className="pd-pal-lab-hero__actions" aria-label="Akcje Laboratorium">
      ...
      <nav aria-label="Oś procesu Laboratorium" className="pd-pal-lab-process">
        {papaLabWorkbenchModes.map((mode) => (
          <button
            aria-current={activeMode === mode.id ? 'step' : undefined}
            key={mode.id}
            onClick={() => onModeChange(mode.id)}
            type="button"
          >
            <Icon decorative name={mode.icon} size={16} />
            <span>{mode.label}</span>
          </button>
        ))}
      </nav>

      <div className="pd-pal-lab-surface">
        <aside className="pd-pal-lab-rail" aria-label="Warstwy Laboratorium">
          {([
            ['sidebar', 'Analizy', 'menu'],
            ['context', 'Kontekst', 'data'],
            ['evidence', 'Dowody', 'security'],
            ['quality', 'Ryzyko', 'warning'],
            ['actions', 'Decyzja', 'decisions'],
            ['library', 'Biblioteka', 'integration'],
        ...
        <div className="pd-pal-lab-main">
          <div className="pd-pal-lab-view-tabs" aria-label="Widok analizy" role="tablist">
          ...
          <section className="pd-pal-lab-answer" aria-label="Główna analiza Papa Asystenta">
            <div className="pd-pal-lab-answer__intro">
              <span>Odpowiedź Papa Asystenta</span>
              <h2>{response.recommendations[0]}</h2>
              <p>{activeModeDefinition.summary}</p>
            </div>

            <div className="pd-pal-lab-facts">
              <ResponseList label="Fakty" values={response.facts} />
              <ResponseList label="Interpretacja" values={response.interpretations} />
          ...
          <footer className="pd-pal-lab-dock" aria-label="Skrót kontekstu i kontroli">
            <button onClick={() => onLayerChange('context')} type="button">
              <span>Kontekst</span>
              <strong>4 źródła</strong>
            </button>
            <button onClick={() => onLayerChange('evidence')} type="button">
              <span>Dowody</span>
              <strong>lineage OK</strong>
            </button>
            <button onClick={() => onLayerChange('quality')} type="button">
              <span>Jakość</span>
              <strong>92%</strong>
            </button>
            <button onClick={() => onLayerChange('actions')} type="button">
              <span>Decyzja</span>
              <strong>needsReview</strong>
            </button>
            <button aria-pressed={isFocusMode} onClick={onFocusModeToggle} type="button">
              <span>Skupienie</span>
              <strong>{isFocusMode ? 'włączone' : 'wyłączone'}</strong>
            </button>
          </footer>
        </div>
      </div>
...
function layerLabel(layer: PapaLabWorkbenchLayerId): string {
  switch (layer) {
    case 'actions':
      return 'Decyzja i audit trail';
    case 'context':
      return 'Context Basket';
    case 'evidence':
      return 'Dowody i lineage';
    case 'library':
      return 'Biblioteka artefaktów';
    case 'quality':
      return 'Jakość danych i ryzyko';
    case 'sidebar':
      return 'Analizy i historia';
  }
}
