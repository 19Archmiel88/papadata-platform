const shiftOverviewDate = (value: string, days: number) => { const date = new Date(`${value}T12:00:00Z`); date.setUTCDate(date.getUTCDate() + days); return date.toISOString().slice(0, 10); };
import {
  decisionDomains,
  type Decision,
  type DecisionEvent,
...
export const decisionLocalDay = (at: string | Date, timezone = 'Europe/Warsaw') =>
...
export function applyDecisionEvent(
  ...
  const today = decisionLocalDay(event.at, timezone);
  ...
  if (cmd.type === 'create') {
    ...
    next = {
      ...
      evidencePath: event.context?.sourcePath ?? null,
          ...
          cmd.date <
            decisionLocalDay(
              store.activity
                ...
                .at(-1)?.at ?? current.createdAt, timezone,
            )
        )
