...
component_id: FunnelStep
---
# FunnelStep

## Cel i odpowiedzialność
`FunnelStep` rozwiązuje jeden określony problem interfejsu i nie przejmuje odpowiedzialności ekranu ani domenowego API. Kontrakt jest stanem docelowym wymagającym implementacji i testów.
...
`contracts/components/funnelstep.ts` re-eksportuje `FunnelStepProps` z `contracts/domain-component-contracts.ts`; nie istnieje drugi konkurencyjny interfejs.

| Pole / kontrakt | Typ | Reguła |
|---|---|---|
| `FunnelStepProps` | import z `domain-component-contracts.ts` | Szczegółowe pola domenowe są zdefiniowane w jednym pliku kanonicznym. |
