...
import { ProductionDatabase, SecurityRepository, MfaEnrollmentConflict } from "@papadata/database";
...
export class TotpService{
 ...
 async enroll(input:{tenantId:string;userId:string;accountName:string}):Promise<{secret:string;otpauthUri:string;recoveryCodes:readonly string[]}>{const secret=encode(randomBytes(20));const recoveryCodes=Array.from({length:10},()=>randomBytes(8).toString("hex"));const recoveryCodeHashes=recoveryCodes.map(value=>this.hashRecoveryCode(value));try{await this.repository.saveMfaEnrollment({tenantId:input.tenantId,userId:input.userId,encryptedSecret:this.encrypt(secret),recoveryCodeHashes});}catch(cause){if(cause instanceof MfaEnrollmentConflict)throw new ConflictException('MFA is already active. Use the protected disable flow before configuring a new factor.');throw cause;}return{secret,otpauthUri:`otpauth://totp/${encodeURIComponent(`PapaData:${input.accountName}`)}?secret=${secret}&issuer=PapaData&algorithm=SHA1&digits=6&period=30`,recoveryCodes};}
