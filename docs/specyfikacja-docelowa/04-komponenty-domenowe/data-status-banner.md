...
component_id: DataStatusBanner
---
# DataStatusBanner

## Cel i odpowiedzialność
`DataStatusBanner` rozwiązuje jeden określony problem interfejsu i nie przejmuje odpowiedzialności ekranu ani domenowego API. Kontrakt jest stanem docelowym wymagającym implementacji i testów.
...
`contracts/components/datastatusbanner.ts` re-eksportuje `DataStatusBannerProps` z `contracts/domain-component-contracts.ts`; nie istnieje drugi konkurencyjny interfejs.

| Pole / kontrakt | Typ | Reguła |
|---|---|---|
| `DataStatusBannerProps` | import z `domain-component-contracts.ts` | Szczegółowe pola domenowe są zdefiniowane w jednym pliku kanonicznym. |
