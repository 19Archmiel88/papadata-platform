...
export function ordersFixture(range:CommerceRange):OrdersPortfolio {
  const records:CommerceOrder[]=Array.from({length:12},(_,i)=>{const gross=120+i*34,date=businessDateShift(range.to,-Math.min(i,Math.max(0,Math.floor((Date.parse(range.to)-Date.parse(range.from))/86400000))));
    return {id:`${sourceId}:demo-${i+1}`,externalId:`demo-${i+1}`,number:`DEMO-${String(1200+i)}`,sourceId,provider:'woocommerce',currency:'PLN',orderedAt:date+'T10:00:00Z',dateBasis:'source',observedAt:range.to+'T10:00:00Z',status:i===2?'pending':i===3?'failed':i===4?null:'completed',payment:i===2?'pending':i===3?'failed':i===4?'unknown':'paid',fulfillment:i===2?'pending':i===3?'unknown':'fulfilled',paymentMethod:i%2?'karta':'przelew',paidAt:i>1?null:date+'T10:02:00Z',completedAt:i>1?null:date+'T12:00:00Z',shippingMethod:'Kurier',gross,net:i===4?null:gross/1.23,tax:i===4?null:gross-gross/1.23,discount:0,shippingCharged:12,customerPseudonym:`Klient demo ${i+1}`,qualified:i!==2&&i!==3,
