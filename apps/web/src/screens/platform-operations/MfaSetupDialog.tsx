...
export function MfaSetupDialog({open,onClose,onEnroll,onConfirm}:{open:boolean;onClose:()=>void;onEnroll:()=>Promise<MfaEnrollment>;onConfirm:(code:string)=>Promise<void>}){
 ...
 {!enrollment?<><p>{t('Wygeneruj sekret i zapisz kody odzyskiwania w menedżerze haseł. Dane pozostają tylko w pamięci tego widoku; nie są zapisywane w localStorage.','Generate a secret and store recovery codes in your password manager. These details stay only in this view\'s memory, not in localStorage.')}</p><Button disabled={busy} onClick={()=>{setBusy(true);setError(null);void onEnroll().then(setEnrollment).catch(cause=>setError(cause instanceof Error?cause.message:'Enrollment failed')).finally(()=>setBusy(false));}}>{t('Rozpocznij konfigurację','Start setup')}</Button></>:<form onSubmit={e=>{e.preventDefault();if(!saved||busy)return;setBusy(true);setError(null);void onConfirm(code).then(()=>{setEnrollment(null);onClose();}).catch(cause=>setError(cause instanceof Error?cause.message:'Confirmation failed')).finally(()=>setBusy(false));}}>
 <p>{t('Dodaj konto PapaData do aplikacji uwierzytelniającej jako TOTP, 6 cyfr, okres 30 sekund.','Add PapaData to your authenticator as TOTP, 6 digits, 30-second period.')}</p>
 <label>{t('Sekret do ręcznego wprowadzenia','Manual setup secret')}<input readOnly value={enrollment.secret} autoComplete="off" spellCheck={false}/></label>
 <details><summary>{t('Kody odzyskiwania jednorazowego użytku','One-time recovery codes')}</summary><p>{t('Nie udostępniaj tych kodów. Nie pojawią się ponownie po zamknięciu widoku.','Do not share these codes. They will not be shown again after closing this view.')}</p><pre>{enrollment.recoveryCodes.join('\n')}</pre></details>
 <label className="pd-operations__check"><input type="checkbox" checked={saved} onChange={e=>setSaved(e.target.checked)}/>{t('Zapisałem kody odzyskiwania w bezpiecznym miejscu.','I saved the recovery codes in a secure location.')}</label>
 ...
 <Button type="submit" disabled={busy||!saved||code.length!==6}>{t('Potwierdź i włącz MFA','Confirm and enable MFA')}</Button></form>}
