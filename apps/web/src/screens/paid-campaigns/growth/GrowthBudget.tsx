...
export function GrowthBudget({ data, onSave, onReload }: {
    ...
    <div className="pd-product-data__chart" role="img" aria-label={t('Plan i wydatki narastająco; dane w tabeli poniżej', 'Cumulative plan and spend; data table below')}><ResponsiveContainer width="100%" height="100%"><LineChart data={[...projection.points]}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date"/><YAxis /><Tooltip /><Line type="monotone" dataKey="planned" name={t('Plan', 'Plan')} stroke="var(--pd-growth-secondary)" dot={false}/><Line type="monotone" dataKey="actual" name={t('Wydatki', 'Spend')} stroke="var(--pd-growth-accent)" dot={false} connectNulls={false}/></LineChart></ResponsiveContainer></div>
