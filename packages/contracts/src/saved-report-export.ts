import { reportDate, reportNumber } from './saved-report-presentation.js';
import type { ReportVersion, SavedReport } from './saved-reports.js';
...
const csvCell = (v: unknown) => {
  const value = typeof v === 'string' && /^[\s\x00-\x1f]*[=+@\-]/.test(v) ? `'${v}` : String(v ?? '');
...
export function reportCsv(v: ReportVersion) {
  ...
  return (
    ...
    [
      ...
      ['Wersja', v.number],
      ['Dane', s.mode === 'demo' ? 'Przykładowe' : s.mode === 'imported' ? 'Importowane' : 'Produkcyjne'],
      ['Zakres', s.scopeLabel],
...
export type SavedReportDownload = {readonly filename:string;readonly content:string;readonly mediaType:string;readonly reportId:string;readonly version:number;readonly format:'csv'|'html'|'json'};
