...
component_id: PlanPerformance
---
# PlanPerformance

## Cel i odpowiedzialność
`PlanPerformance` rozwiązuje jeden określony problem interfejsu i nie przejmuje odpowiedzialności ekranu ani domenowego API. Kontrakt jest stanem docelowym wymagającym implementacji i testów.
...
`contracts/components/planperformance.ts` re-eksportuje `PlanPerformanceProps` z `contracts/domain-component-contracts.ts`; nie istnieje drugi konkurencyjny interfejs.

| Pole / kontrakt | Typ | Reguła |
|---|---|---|
| `PlanPerformanceProps` | import z `domain-component-contracts.ts` | Szczegółowe pola domenowe są zdefiniowane w jednym pliku kanonicznym. |
